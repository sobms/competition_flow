	...
	def _create_streaming_dataset(self, dataset: Any, streaming_spec: Dict[str, Any], mode: str):
		...
		if item_cols or user_cols:
			...
			try:
				...
				if user_cols:
					users_files_sql = "[" + ",".join([f"'{p}'" for p in user_shards]) + "]" if user_shards else "[]"
					sel_users_cols = ", ".join([user_field] + user_cols)
					con_reg.execute(
						f"CREATE OR REPLACE VIEW users_all AS SELECT {sel_users_cols} FROM read_parquet({users_files_sql})"
					)
			finally:
				con_reg.close()
		...
		outer = self
		class _DSSMIterable(IterableDataset):
			...
			def __iter__(self) -> Iterator[Interaction]:
				...
				try:
					for inter_path in tqdm(inter_shards, desc="Processing inter shards"):
						# SQL: join items/users, filter positives, order by user/time
						if con_main is None:
							con = duckdb.connect()
						else:
							con = con_main
						it_sel = ", ".join([f"it.{c}" for c in item_cols]) if item_cols else ""
						ud_sel = ", ".join([f"u.{c}" for c in user_cols]) if user_cols else ""
						...
						if ud_sel:
							extra += ", " + ud_sel
						base = f"FROM read_parquet('{inter_path}') i "
						if item_cols:
							base += f"LEFT JOIN items_all it USING ({item_field}) "
						if user_cols:
							base += f"LEFT JOIN users_all u USING ({user_field}) "
						sql = (
						...
						# slice into mini-batches with tqdm logging
						...
						for start in _batch_pbar:
							...
							if item_cols:
								feat_cols += [c for c in item_cols if c]
							if user_cols:
								feat_cols += [c for c in user_cols if c]
							buf_feats: Dict[str, List[Any]] = {}
							for k in feat_cols:
								if k in sb.schema.names:
									buf_feats[k] = sb.column(k).to_pandas().iloc[keep].tolist()
							feat_runtime = inter_feature_cols + item_feature_cols + user_feature_cols
